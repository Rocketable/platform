...
export function appRouter(layer: AppLayer) {
  ...
  return t.router({
    listSessionEntries: identified.input(z.object({ id: z.string() })).query(({ ctx, input }) =>
      run(
        Effect.gen(function* () {
          const rc = yield* Rocketclaw;
          return yield* rc.listSessionEntries(ctx.principal, input.id);
        }),
      ),
    ),
    loadSessionEntries: identified.input(z.object({ id: z.string() })).query(({ ctx, input }) =>
      run(
        Effect.gen(function* () {
          const rc = yield* Rocketclaw;
          return yield* rc.loadSessionEntries(ctx.principal, input.id);
        }),
      ),
    ),
    deleteSessionEntries: identified.input(z.object({ id: z.string() })).mutation(({ ctx, input }) =>
      run(
        Effect.gen(function* () {
          const rc = yield* Rocketclaw;
          return yield* rc.deleteSessionEntries(ctx.principal, input.id);
        }),
      ),
    ),
