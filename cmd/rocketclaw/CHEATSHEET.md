...
Agents use filesystem, shell, fetch, patch, and outbound MCP through the Code Mode `execute` tool (not as separate top-level tools). After the right permission grants, call `execute` with a short Starlark script (`def main()`). Inside the script: host builtins `read(...)`, `apply_patch(...)`, `glob(...)`, `grep(...)`, `webfetch(...)`, `shell(command=r"…")` with their normal permission buckets and subjects (including multi-subject `shell`); RocketClaw platform tools such as `ask_user_question(...)`, `rocketclaw_reload(...)`, `rocketclaw_update_goal(...)` (also still available top-level when eligible); MCP as `server_toolname(...)` after `permission.mcp` (permission subjects use raw `server.tool`). Use raw strings for shell, regex, and paths: `r"..."` is single-line; multiline or heredocs need `r'''...'''`. Ordinary `"..."` rejects unknown escapes such as `\(`, `\.`, `\$`. `$` is valid inside a closed string. The script is parsed before host tools run; a `codemode.star` error means nothing ran. `shell(...)` results are text-like (`str(result)` before find/split). Discover tools with in-script `search(query="", namespace="", offset=0, limit=10)` (JSON items with path/description/signature/callable). A short Code Mode catalog is also in the system prompt. Skills, `task`, hosted `websearch`, and RocketClaw platform tools stay top-level when eligible.
...
| `execute` | Shown when the agent has MCP grants, code-mode host grants (`read`/`edit`/`shell`/…), and/or RocketClaw platform-tool grants. The only model entry for FS/shell/fetch/patch/MCP; platform tools remain top-level too. | Entry gate uses mcp subjects when present, otherwise a matching host or platform-tool permission subject. Nested calls re-check real buckets (read/edit/shell/mcp/rocketclaw/…). Not RocketClaw auto-allowed. | Code Mode: runs a short Starlark script with required `code`. Define `def main()`; call host tools as `read(filePath="…")` / `shell(command=r"…")`, platform tools as `ask_user_question(question="…")` / `rocketclaw_reload(reason="…")`, MCP as `server_toolname(**kwargs)`, concurrency via `gather`/`map`/`race`/`race_first` (default concurrency 16, max 64), and `search(query="", namespace="", offset=0, limit=10)` to discover tools and concurrency builtins. Use raw strings (`r"..."` is single-line; multiline needs `r'''...'''`) for shell, regex, and paths. Parsed before host tools; failed wrappers are not evidence. Example: `def main():\n  return gather([lambda: read(filePath="a"), lambda: read(filePath="b")])`. Connects MCP per call, then closes. |
...
| `shell` | Parsed shell call expressions for nested `shell` inside `execute`. Multi-command scripts need every parsed call allowed. |
