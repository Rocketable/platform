import { remainingTokens, type Goal } from "./goal.js"

export function continuation(goal: Goal) {
...
Progress visibility:
If the next work is meaningfully multi-step, keep a concise plan tied to the real objective. Keep the plan current as steps complete or the next best action changes. Skip planning overhead for trivial one-step progress, and do not treat a plan update as a substitute for doing the work.
...
export function objectiveUpdated(goal: Goal) {
  const remaining = remainingTokens(goal)
...
- Tokens used: ${goal.tokensUsed}
- Token budget: ${goal.tokenBudget ?? "none"}
- Tokens remaining: ${remaining === undefined ? "unknown" : remaining}
...
export function budgetLimit(goal: Goal) {
