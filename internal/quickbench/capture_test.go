...
func TestCaptureFromSession(t *testing.T) {
	...
	assert.Equal(t, "user", bar.Variations[0].Transcript[len(bar.Variations[0].Transcript)-1].Role)
	require.Len(t, bar.Tools, 1)
	assert.Equal(t, "read", bar.Tools[0].Name)
	assert.Equal(t, "file-body", bar.Tools[0].Response)
