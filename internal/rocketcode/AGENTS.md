...
- Avoid single-use functions. If a helper is only called once, inline it by default. Only extract it when it is reused or materially clarifies a dense block. If it is single-use and single-line, inlining is mandatory.
