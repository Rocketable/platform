...
- Newly persisted provider-routed turns store provider-qualified model names. Runtime model selection accepts `openai/<model>`, `anthropic/<model>`, and `openai-compatible/<provider>/<model>` only. Unprefixed model names and empty model reference parts are construction errors that prevent RocketCode startup. Every loaded agent, including guardrail, reviewer, and subagent agents, must declare non-empty `model` frontmatter, and each declared model must validate against configured providers during startup. Runtime/default models do not fill missing agent model declarations.
- Compaction replay items remain Responses-shaped `compaction` items in durable replay for every provider. Provider-native encrypted compaction payloads are preserved when supplied. Readable summary metadata, recent-context metadata, origin provider, and origin mode are preserved when available so provider adapters can project compaction boundaries into the target provider's input format.
...
- OpenAI, OpenAI-compatible, and Anthropic are peer provider families for RocketCode runtime behavior. A capability implemented for one provider family must be implemented for the other provider families with provider-appropriate mechanics unless an ADR explicitly exempts a provider family from that specific capability.
- Provider-native differences are implementation details, not product capability differences. When a provider lacks a native feature, RocketCode must provide an equivalent local, client-side, adapter-level, or provider-specific mechanism when feasible. Examples include client-side summarization for APIs without native compaction, provider-specific tool-call adapters, provider-specific structured-output mapping, and replay projection into the target provider's accepted input shape.
- Shared provider-facing capabilities must be represented by shared interfaces, construction contracts, or equivalent compile-time structure that makes provider participation explicit. New unsupported-provider branches, silent capability drops, or provider-specific no-op paths are not allowed unless an ADR explicitly removes that provider family from the capability and states the user-visible and internal consequences.
- Tests for shared provider-facing capabilities must cover OpenAI, OpenAI-compatible, and Anthropic behavior separately when practical. If exact output cannot be shared because provider APIs differ, tests must verify the provider-appropriate equivalent contract.
...
- Each model cycle routes to the provider selected by the active model string. Provider adapters must preserve prompt framing, local tool call dispatch, tool-output continuation, diagnostics, and replay semantics. OpenAI-compatible `chat_completions` adapters preserve system, developer, user, assistant, and tool semantics as far as the target API supports. `openai-compatible` model strings select a named compatible provider whose configured mode is either `responses` or `chat_completions`.
- History before the latest compaction point is pruned so replay starts from the compaction boundary when one exists.
- Compaction steering, when configured, is appended as developer text after stored compaction items. OpenAI Responses context compaction remains provider-specific. Anthropic runtime turns enable Anthropic beta server-side compaction, persist returned Anthropic compaction blocks in replay, and replay those blocks to Anthropic on later requests so the provider can continue from its summary boundary. OpenAI-compatible `responses` mode uses that compatible provider's Responses API and native compaction behavior. OpenAI-compatible `chat_completions` mode performs client-side compaction with the same selected compatible provider and model, stores the result as a Responses-shaped compaction item, and replays readable compactions as lower-authority user checkpoint context.
- OpenAI-compatible `chat_completions` successful responses whose provider-reported total token usage reaches the configured compact threshold trigger ChatGPT-style automatic client-side compaction before the response is persisted. Automatic compaction prepends the compacted boundary to the response output, preserves safe-block semantics by skipping unanswered tool calls, avoids duplicate compaction when the response already contains a compaction item, stores origin metadata as `origin_provider:"openai-compatible"`, `origin_compatible_provider`, and `origin_mode:"chat_completions"`, and lets later turns prune history before that boundary.
...
- OpenAI-compatible `chat_completions` context-length recovery triggers client-side compaction before the turn fails. Recovery compacts older safe replay blocks progressively, retries after each successful compaction, preserves tool calls with their corresponding tool outputs, does not compact unanswered tool calls, keeps the newest active block unchanged, and persists only compaction items needed for a successful retry.
- Before sending stored conversation history to a provider, RocketCode must translate replay items into the target provider's supported input shape. This replay projection is a shared provider-family capability, not an OpenAI-only behavior. Native encrypted compaction items may be sent only back to the exact provider and mode that can use them. When the target provider cannot use a native compaction item, RocketCode must preserve the conversation boundary by converting readable summaries into lower-authority user checkpoint messages. If only unreadable encrypted compaction data is available, RocketCode must provide a rehydration notice rather than silently dropping the boundary. Anthropic targets must replay Anthropic compaction blocks when possible and otherwise preserve readable summary context. All provider adapters must preserve continuation semantics even when they cannot consume the original native replay item.
- OpenAI surfaced provider errors are classified with strong provider error types so RocketCode owns user-visible retry diagnostics and backoff. Rate-limit handling follows Codex semantics by default: direct HTTP 429 transport errors do not retry, `usage_limit_reached` and `usage_not_included` are terminal, and retryable failed/stream responses with `rate_limit_exceeded` retry with a bounded retry budget. As a provider-specific extension, direct HTTP 429 errors whose SDK-provided `code` or `type` is `too_many_requests` are retryable. OpenAI-compatible `chat_completions` rate-limit errors use the same retry semantics when SDK status, code, or type fields expose retryable status. Retry delay uses typed HTTP retry metadata when present, otherwise bounded exponential backoff is used. Provider messages are diagnostic text, not retry-control input.
