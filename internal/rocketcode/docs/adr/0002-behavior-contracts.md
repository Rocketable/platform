...
- Newly persisted turns store unprefixed OpenAI model IDs. Runtime model selection accepts unprefixed OpenAI model IDs and legacy `openai/<model>` aliases normalized to `<model>`. Empty runtime/default model resolves to `gpt-5.5`. Other provider-qualified model names, Anthropic model names, and empty explicit model reference parts are construction errors that prevent RocketCode startup. Loaded agents, including guardrail, reviewer, and subagent agents, must declare non-empty `model` frontmatter; missing or empty agent models are construction errors rather than inheriting the runtime/default model.
