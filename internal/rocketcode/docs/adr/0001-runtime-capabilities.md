...
| Rate limits and context limits | RocketCode classifies surfaced provider errors with strong provider error types and owns user-visible provider retry diagnostics and backoff. Rate-limit behavior follows Codex by default: direct HTTP 429 transport responses are classified but are not retried, `usage_limit_reached` and `usage_not_included` are terminal, and retryable OpenAI Responses failed/stream errors use strong provider error types for `rate_limit_exceeded`. As a provider-specific extension, direct HTTP 429 errors whose SDK-provided `code` or `type` is `too_many_requests` are retryable because supported providers emit that typed error shape for transient rate limiting. Retryable rate-limit errors use typed HTTP retry metadata when present and otherwise use bounded exponential retry backoff. Rate-limit retries are bounded rather than infinite. OpenAI Responses context-length API errors trigger explicit progressive compaction of older safe replay blocks and retry before surfacing a runtime error. Other failed responses and API errors surface as runtime errors, with provider diagnostics emitted when diagnostics are enabled. |
