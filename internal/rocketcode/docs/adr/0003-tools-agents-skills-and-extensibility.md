...
| --- | --- | --- | --- |
| `websearch` | `websearch` | `*` | Hosted OpenAI web-search tool, exposed for OpenAI model requests. |
...
- Agent YAML frontmatter is required and must be a mapping. Known fields are `description`, `model`, `reasoningEffort`, `verbosity`, `maxRecursion`, `guardrail`, and `permission`. The `model` field is required, must be non-empty, and must be an unprefixed OpenAI Responses model ID such as `gpt-5.5` or a legacy `openai/<model>` alias normalized to `<model>`. Missing or empty `model` is invalid and does not inherit the runtime/default model. Unknown fields remain in `Frontmatter`.
...
- The guardrail response must be strict JSON containing `approved` and `reason` fields. Invalid JSON fails closed. Guardrail model requests use the OpenAI Responses structured-output path.
...
- The embedded guardian has read-oriented default permissions only: `read`, `glob`, and `grep` are allowed for all subjects; shell, edit, task, web, skill, and custom-tool access are not allowed by default. The embedded guardian uses `Config.AutoApproverModel` when non-empty, otherwise the resolved runtime/default model, and uses `low` reasoning effort for Codex parity and to keep automatic permission review within its timeout budget.
...
- Prompt and tool attachments become OpenAI Responses API image or file content items in RocketCode's internal replay shape and must fail clearly rather than silently dropping unsupported attachment content.
