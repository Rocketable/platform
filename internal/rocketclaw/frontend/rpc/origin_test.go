...
func TestCreatingCronLocatorIgnoresLaterOneOffOnSlackThread(t *testing.T) {
	...
	locator, ok := creatingCronLocator("slack-thread:C1:1.2", "", entries)
	require.False(t, ok)
	require.Empty(t, locator)

	locator, ok = creatingCronLocator("slack-thread:C1:1.2", backend.ThreadCreatedByCron, entries)
	require.True(t, ok)
	require.Equal(t, later, locator)

	webSource := "web:" + later
	locator, ok = creatingCronLocator(webSource, "", nil)
	require.True(t, ok)
	require.Equal(t, later, locator)
}
