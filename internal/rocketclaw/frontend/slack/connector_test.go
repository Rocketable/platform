...
func TestNewUsesInjectedDependencies(t *testing.T) {
	...
	c := New(&config.SlackConfig{BotToken: "xoxb-test", AppToken: "xapp-test"}, bus, nil, inertConversationBackend{}, inertSideAskHost{}, testLogger())
	c.SetCron(inertOneOffCronjobs{})

	_, err := c.conv.WorkflowDescriptions()
	require.Error(t, err)
	_, err = c.conv.ScheduledMessages("slack-thread:D123:111.222")
	require.Error(t, err)
...
func TestDirectMessagesHaveNoEffect(t *testing.T) {
	connector := New(
		...
		newTestBus(), nil, inertConversationBackend{}, inertSideAskHost{},
		testLogger(),
	)
...
func TestEventLoopRoutesEventsAPI(t *testing.T) {
	...
	require.Eventually(t, func() bool { return len(convTurns(connector)) == 1 }, time.Second, time.Millisecond)
	assert.Equal(t, "please check this", convTurns(connector)[0].Text)
...
func TestExternalMCPRelayUsesAnswerPlaceholderForStackedReply(t *testing.T) {
	...
	final := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", "first answer", protocol.OutputTargetSlack)
	final.TurnID = "turn-1"
	final.Complete = true
...
func TestExternalMCPRelayCreatesAnswerPlaceholderUpFront(t *testing.T) {
	...
	thinking := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", "", protocol.OutputTargetSlack)
...
func TestExternalMCPRelayTailResponseUpdatesAnswerPlaceholder(t *testing.T) {
	...
	final := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", "tail answer", protocol.OutputTargetSlack)
	final.TurnID = "turn-1"
	final.Complete = true
	final.ExternalConversationID = "public-conversation"
	final.Agent = "private-agent"
...
func TestExternalMCPResponseBlocksSurviveChunking(t *testing.T) {
	...
	final := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", text, protocol.OutputTargetSlack)
	final.TurnID = "turn-1"
	final.Complete = true
	final.ExternalConversationID = "public-conversation"
	final.Agent = "private-agent"
...
func TestExternalMCPRelayStackedTailResponseUpdatesAnswerPlaceholder(t *testing.T) {
	...
	final := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", "second answer", protocol.OutputTargetSlack)
	final.TurnID = "turn-2"
	final.Complete = true
	final.ExternalConversationID = "public-conversation"
	final.Agent = "private-agent"
...
func TestSendCronjobChannelThreadPostsBodyInRoot(t *testing.T) {
	...
	connector := newTestConnectorWithOptions(server.URL, nil, nil, nil, nil)
...
func TestSendResponseCronjobUsesRecurringLayout(t *testing.T) {
	...
	oneOff := protocol.NewOutboundMessage(protocol.SourceSystem, "thread", "final payload", protocol.OutputTargetSlack)
	oneOff.Complete = true
	oneOff.Cronjob = &protocol.CronjobMessage{RelativePath: "cron/daily.md", Agent: "planner", RanAt: "2000-01-02T03:04:05Z"}
	oneOff.SlackReply = &protocol.SlackReplyTarget{ChannelID: "#triage", ThreadTS: "999.111"}
...
func TestSendCronjobChannelThreadPostsAttachmentOnlyInRootThread(t *testing.T) {
	...
	connector := newTestConnectorWithOptions(server.URL, nil, nil, newThreadRouterStub(), nil)
	require.NoError(t, connector.SendCronjobChannelThread(context.Background(), "#triage", "cron/daily.md", "planner", "2000-01-02T03:04:05Z", "", []protocol.OutboundAttachment{{Name: "report.txt", Data: []byte("report body")}}))
...
func TestSendCronjobChannelThreadReportsSlackFailures(t *testing.T) {
	...
	t.Run("attachments", func(t *testing.T) {
		...
		connector := newTestConnector(server.URL)
		err := connector.SendCronjobChannelThread(context.Background(), "#triage", "cron/daily.md", "planner", "2000-01-02T03:04:05Z", "", []protocol.OutboundAttachment{{Name: "report.txt", Data: []byte("report")}})
...
func TestSendResponseKeepsHumanThinkingTaskCardLifecycle(t *testing.T) {
	...
	progress := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "", protocol.OutputTargetSlack)
	...
	partial := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "Partial answer", protocol.OutputTargetSlack)
	partial.TurnID = "turn-1"
	partial.SlackReply = reply
	require.NoError(t, connector.SendResponse(t.Context(), partial))

	final := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "Final answer", protocol.OutputTargetSlack)
	final.TurnID = "turn-1"
	final.Complete = true
...
func TestSendResponseCompletesThinkingPlanStreamAfterUnchangedAnswer(t *testing.T) {
	...
	answer := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "Final answer", protocol.OutputTargetSlack)
...
func TestSendResponseKeepsDeliveredAnswerWhenTaskStreamStopFails(t *testing.T) {
	...
	connector.stacks[key] = []slackBufferedMessage{{
		...
		Content:         protocol.InboundContent{Text: "second"},
		Reply:           bufferedReply,
	}}
...
func TestAbortResponseStopsTaskStreamBeforeDeleting(t *testing.T) {
	...
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "", protocol.OutputTargetSlack)
	msg.TurnID = "turn-1"
...
func TestFlushProgressFallsBackToPlanUpdateWhenStreamEnded(t *testing.T) {
	for _, slackError := range []string{"message_not_in_streaming_state", "stopped_by_user"} {
		t.Run(slackError, func(t *testing.T) {
			...
			agent := protocol.AgentUpdate{CallID: "run/agent/000000", PhaseID: "run/phase/000000/audit", Label: "failure-trace", Activity: "read: <https://example.com/report|report>"}
			secondAgent := protocol.AgentUpdate{CallID: "run/agent/000001", PhaseID: "run/phase/000000/audit", Label: "canonical-owner", Activity: "grep: ownership"}
...
func TestBufferProgressDoesNotResurrectEndedStream(t *testing.T) {
	for _, buffer := range []string{"activity", "phase"} {
		t.Run(buffer, func(t *testing.T) {
			...
			if buffer == "activity" {
				progress := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "", protocol.OutputTargetSlack)
				progress.TurnID = "run"
...
func TestSendResponseClampsThinkingToSlackLimit(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	first := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "", []protocol.OutputTarget{protocol.OutputTargetSlack}...)
	first.TurnID = "turn-1"
	...
	second := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "", []protocol.OutputTarget{protocol.OutputTargetSlack}...)
...
func TestSendResponseUsesGoalBlocksForGoalAnswers(t *testing.T) {
	...
	body := "Progress summary: I counted 1. Current state: 1 of 10. Next concrete step: count 2 on the next turn."
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", body, protocol.OutputTargetSlack)
	msg.TurnID = turnID
	msg.Complete = true
	msg.GoalTurn = true
	...
	done := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "shipped", protocol.OutputTargetSlack)
	done.TurnID = turnID
	done.Complete = true
	done.GoalTurn = true
...
func TestHandleBroadcastRelayCleanup(t *testing.T) {
	connector := newTestConnector("http://slack.test")
	reply := make(chan protocol.BroadcastReply, 1)
	ack := connector.HandleBroadcast(t.Context(), &protocol.Broadcast{
		RelayCleanup:  &protocol.InboundMessage{SlackReply: &protocol.SlackReplyTarget{ChannelID: "C1", MessageTS: "1.1", ThreadTS: "1.1"}},
		RelayResponse: reply,
	})
	assert.Equal(t, protocol.BroadcastHandled, ack.Status)

	select {
	case <-reply:
	default:
		t.Fatal("expected relay response")
	}
}
...
func TestSetMCPAttachmentOnlyResponseText(t *testing.T) {
	msg := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "conversation", "")
	msg.Complete = true
	...
	empty := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "conversation", "")
	empty.Complete = true
...
func TestSendResponseSplitsLongFinalAnswerIntoThreadMessages(t *testing.T) {
	...
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", longText, protocol.OutputTargetSlack)
...
func TestSendResponseUpdatesTailAnswerPlaceholder(t *testing.T) {
	...
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "thread answer", protocol.OutputTargetSlack)
	msg.TurnID = "turn-thread"
	msg.Complete = true
	msg.Agent = "main"
	msg.SlackReply = replyTarget
...
func TestSendResponseUpdatesNonTailAnswerPlaceholder(t *testing.T) {
	...
	thinking := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "", protocol.OutputTargetSlack)
	thinking.TurnID = "turn-thread"
	thinking.ProgressText = "thinking"
	thinking.SlackReply = first
	require.NoError(t, connector.SendResponse(context.Background(), thinking))

	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "first answer", protocol.OutputTargetSlack)
	msg.TurnID = "turn-thread"
	msg.Complete = true
...
func TestSendResponseDeletesThinkingStreamWithoutProgress(t *testing.T) {
	...
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "final answer", protocol.OutputTargetSlack)
	msg.TurnID = "turn-thread"
	msg.Complete = true
	msg.SlackReply = replyTarget
	require.NoError(t, connector.SendResponse(t.Context(), msg))
...
func TestCreateReplyPlaceholdersStartsThinkingPlanStreamBeforeUnchangedAnswer(t *testing.T) {
	...
	require.Len(t, convTurns(connector), 1)
	assert.Equal(t, "hello", convTurns(connector)[0].Text)
...
func TestSendResponseFallbackUsesReplyRecipientForPlanStream(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	progress := protocol.NewOutboundMessage(protocol.SourceSystem, "slack-thread:C123:111.1", "", protocol.OutputTargetSlack)
...
func TestRecipientlessThinkingKeepsTaskCard(t *testing.T) {
	...
	progress := protocol.NewOutboundMessage(protocol.SourceSystem, "test", "", protocol.OutputTargetSlack)
...
func TestSendResponseWithBlankTurnIDDoesNotClaimPendingPlaceholder(t *testing.T) {
	...
	msg := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "metadata", protocol.OutputTargetSlack)
...
func TestSendResponseUploadsAttachmentOnlyMCPResponseToSlackThread(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	msg := protocol.NewOutboundMessage(protocol.SourceExternalMCP, "test", "", protocol.OutputTargetSlack)
	msg.Complete = true
	msg.ExternalConversationID = "public-conversation"
	msg.Agent = "private-agent"
...
func TestSendResponseDoesNotFailWhenAttachmentUploadFails(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	msg := protocol.NewOutboundMessage(protocol.SourceSystem, "test", "final payload", protocol.OutputTargetSlack)
	msg.Complete = true
	msg.SlackReply = &protocol.SlackReplyTarget{ChannelID: "D123", ThreadTS: "111.222"}
...
func TestHandleEventsAPIIncludesNativeForwardedPublicThread(t *testing.T) {
	...
	started := waitTurns(t, connector, 1)
	require.Equal(t, []string{"", "next"}, replyCursors)
	require.Contains(t, started[0].Text, "please check this")
}
...
func TestPreviewOnlyNativeForwardRoutesAuthorizedAppMention(t *testing.T) {
	...
	started := waitTurns(t, connector, 1)
	assert.Equal(t, protocol.TurnPrompt, started[0].Kind)
}
...
func TestHandleMessageEventFinishesStackWhenThreadReplySubmitFails(t *testing.T) {
	...
	assert.Equal(t, slackImmediatePlaceholder, posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
	turns := waitTurns(t, connector, 2)

	var prompts, steers []string

	for _, req := range turns {
		switch req.Kind {
		case protocol.TurnPrompt:
			prompts = append(prompts, req.Text)
		case protocol.TurnSteer:
			steers = append(steers, req.Text)
		case protocol.TurnEnqueue, protocol.TurnCancel, protocol.TurnGoal, protocol.TurnWorkflow:
		}
	}

	assert.Equal(t, []string{"status?"}, prompts)
	assert.Equal(t, []string{"again?"}, steers)
}
...
func TestHandleMessageEventFinishesStackWhenThreadReplyUnhandled(t *testing.T) {
	...
	assert.Contains(t, reactions, "/reactions.add "+slackBufferedReaction+" 171235.9999")
	turns := waitTurns(t, connector, 2)
...
func TestHandleMessageEventBuffersSlackMessagesWhileActive(t *testing.T) {
	...
	assert.Equal(t, []string{"first"}, prompts)
	assert.ElementsMatch(t, []string{"second", "third"}, steers)
	require.Len(t, posted, 3)
	assert.Equal(t, slackImmediatePlaceholder, posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
...
func TestHandleMessageEventPairBusySteersWithoutSlackStack(t *testing.T) {
	...
	assert.Equal(t, protocol.TurnSteer, turns[0].Kind)
	assert.Equal(t, "don't touch the database", turns[0].Text)
...
func TestHandleMessageEventSteerReceiptHasNoPlaceholders(t *testing.T) {
	...
	assert.Equal(t, []string{"first"}, prompts)
	assert.Equal(t, []string{"don't touch the database"}, steers)
...
func TestHandleMessageEventIgnoresThreadParentRedelivery(t *testing.T) {
	...
	assert.Len(t, convTurns(connector), 1)
...
func TestPostWebUserMessageUsesResponsesHeader(t *testing.T) {
	var posted []url.Values
	...
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, newThreadRouterStub(), nil)
	require.NoError(t, connector.PostWebUserMessage(t.Context(), protocol.SlackThreadConversationID("C123", "111.0"), "from web"))
	require.NoError(t, connector.PostWebUserMessage(t.Context(), protocol.WebSessionConversationID("ops"), "skip"))
	require.Len(t, posted, 1)
	assert.Contains(t, posted[0].Get("blocks"), "Web UI - User Message")
	assert.Equal(t, "from web", posted[0].Get("text"))
...
func TestHandleMessageEventEnqueueDuringActiveTurnHasNoPlaceholders(t *testing.T) {
	...
	turns := convTurns(connector)
	require.GreaterOrEqual(t, len(turns), 2)
	assert.Equal(t, protocol.TurnEnqueue, turns[len(turns)-1].Kind)
	assert.Equal(t, "write the changelog", turns[len(turns)-1].Text)
...
func TestHandleMessageEventIdleEnqueueStartsNow(t *testing.T) {
	...
	turns := convTurns(connector)
	require.Len(t, turns, 1)
	assert.Equal(t, protocol.TurnEnqueue, turns[0].Kind)
	assert.Equal(t, "write the changelog", turns[0].Text)
	assert.Contains(t, reactions, "/reactions.add "+slackEnvelopeReaction+" 111.2")
...
func TestHandleMessageEventEnqueueIdenticalTextsRemainDistinct(t *testing.T) {
	...
	assert.Equal(t, protocol.TurnEnqueue, turns[0].Kind)
	assert.Equal(t, "same text", turns[0].Text)
	assert.Equal(t, protocol.TurnEnqueue, turns[1].Kind)
	assert.Equal(t, "same text", turns[1].Text)
}
...
func TestHandleMessageEventWorkflowAllowedWhenQueueExistsWithoutLiveTurn(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, protocol.TurnWorkflow, turns[0].Kind)
	assert.Equal(t, "audit", turns[0].Workflow)
...
func TestHandleMessageEventQueueJumpIndexCard(t *testing.T) {
	...
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, router, nil)
	setLaterWork(connector, []protocol.ThreadQueueItem{
...
func TestHandleMessageEventQueueListsPendingSteersFirst(t *testing.T) {
	...
	router.prepareHandled = true
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, router, nil)
	setLaterWork(connector, []protocol.ThreadQueueItem{{ID: "q1", Message: "later enqueue", SlackChannel: "C123", SlackTS: "111.2"}})
	connector.workspaceURL = "https://example.slack.com"
...
func slackQueueCallback(responseURL, messageTS, userID, actionID, itemID string) slack.InteractionCallback {
	metadata, _ := json.Marshal(slackQueueAction{ChannelID: "C123", ThreadTS: "111.0", ItemID: itemID})
...
func TestAbortResponseReleasesFailedFinalTurnAndPromotesBufferedReply(t *testing.T) {
	...
	connector.stacks[key] = []slackBufferedMessage{{Text: "second", Content: protocol.InboundContent{Text: "second"}, Reply: &protocol.SlackReplyTarget{ChannelID: "C123", MessageTS: "111.2", ThreadTS: "111.0"}}}

	failed := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "first answer", protocol.OutputTargetSlack)
	...
	completed := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "second answer", protocol.OutputTargetSlack)
...
func TestHandleMessageEventConsumesGoalStartRejectionPlaceholder(t *testing.T) {
	...
	assert.Contains(t, reactions, "/reactions.add "+slackRobotReaction+" 171234.5678")
	require.Len(t, posted, 2)
	assert.Equal(t, "_Pursuing Goal (1/5)..._", posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
	turns := waitTurns(t, connector, 1)
	require.Equal(t, protocol.TurnGoal, turns[0].Kind)
	assert.Equal(t, "fix lint", turns[0].Objective)
}
...
func TestHandleMessageEventStartsGoalInExistingManagedThread(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			turns := waitTurns(t, connector, 1)
			assert.Equal(t, protocol.TurnGoal, turns[0].Kind)
			assert.Equal(t, "fix lint", turns[0].Objective)
			assert.Equal(t, 2, turns[0].MaxTurns)
			assert.NotContains(t, turns[0].Objective, "$")
			assert.NotContains(t, turns[0].Objective, "🏁")
...
func TestHandleMessageEventBuffersSteerAfterActiveGoalTurnCompletes(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Len(t, waitTurns(t, connector, 1), 1)

			done := protocol.NewOutboundMessage(protocol.SourceSlack, "test", "Progress summary: started.", protocol.OutputTargetSlack)
			...
			turns := waitTurns(t, connector, 2)
			assert.Equal(t, "don't touch the database", turns[len(turns)-1].Text)
...
func TestHandleMessageEventRejectsDuplicateActiveGoal(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	require.Equal(t, protocol.TurnGoal, turns[0].Kind)
	assert.Equal(t, "another goal", turns[0].Objective)
	assert.Empty(t, turns[0].CheckScript)
}
...
func TestHandleMessageEventStopMarksOriginalTurnStart(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			turns := waitTurns(t, connector, 1)
			assert.Equal(t, protocol.TurnCancel, turns[len(turns)-1].Kind)
			require.GreaterOrEqual(t, len(reactions), beforeReactions+2)
...
func TestHandleAppMentionEventUsesConfiguredChannelAgentAndReaction(t *testing.T) {
	...
	assert.Contains(t, started[0].Text, "please check this")
	require.Len(t, posted, 2)
...
func TestHandleMessageEventRoutesManagedSocialThreadReply(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	_, threadTS, ok := protocol.SlackThreadTarget(turns[0].ID)
	require.True(t, ok)
	assert.Equal(t, "171234.5678", threadTS)
	assert.Equal(t, "refer to <#C111|triage>", turns[0].Text)
	require.Len(t, posted, 2)
	assert.Equal(t, slackImmediatePlaceholder, posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
...
func TestHandleMessageEventSwitchesManagedSocialThreadAgent(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			rec := connector.conv.(*recordingConversationBackend)
			require.GreaterOrEqual(t, len(rec.switches), i+1)
			assert.Contains(t, rec.switches[i], "planner")
...
func TestHandleMessageEventShowsManagedSocialThreadAgentSelector(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Empty(t, connector.conv.(*recordingConversationBackend).switches)
			require.Len(t, posted, i+1)
...
func TestHandleInteractiveSlackAgentSelectorRequiresRequester(t *testing.T) {
	...
	require.Equal(t, []string{protocol.SlackThreadConversationID("C123", "171234.5678") + ":planner"}, connector.conv.(*recordingConversationBackend).switches)
	require.Len(t, posted, 1)
...
func TestHandleMessageEventUsesPerChannelAllowlist(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, "allowed follow up", turns[0].Text)
	require.Len(t, posted, 2)
}
...
func TestHandleMessageEventRoutesSocialThreadReplyPingingBotToo(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, "<@U111> <@U999> please check this", turns[0].Text)
	require.Len(t, posted, 2)
	assert.Equal(t, slackImmediatePlaceholder, posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
}
...
func TestThreadedSocialMentionHandledOnceAndStripped(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, "-- where did that come from?", turns[len(turns)-1].Text)
	require.Len(t, posted, 2)
	assert.Equal(t, slackImmediatePlaceholder, posted[0].Get("text"))
	assert.Equal(t, slackAnswerPlaceholder, posted[1].Get("text"))
	assert.Equal(t, "171234.5678", posted[0].Get("thread_ts"))
...
func TestSlackDollarCommand(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			command, args, ok := protocol.ParseDollarCommand(tt.text)
			assert.Equal(t, tt.ok, ok)
			assert.Equal(t, tt.command, command)
			assert.Equal(t, tt.args, args)
		})
...
func TestWorkflowPhaseUsesStableTaskUpdateAndTerminalPlan(t *testing.T) {
	...
	progress.WorkflowPhase = &protocol.PhaseUpdate{PhaseID: "run-1/phase/audit", Name: "audit", Status: "in-progress", Scheduled: 3, Running: 1, Complete: 2, Details: "checking"}
	require.NoError(t, connector.SendResponse(t.Context(), progress))
	require.NoError(t, connector.flushProgressText(t.Context(), "run-1"))

	final := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "finished", protocol.OutputTargetSlack)
...
func TestSendResponseCompletesFallbackPlanAndPromotesQueuedReply(t *testing.T) {
	...
	connector.stacks[key] = []slackBufferedMessage{{Text: "queued reply", recipientTeamID: "T123", recipientUserID: "U123", Content: protocol.InboundContent{Text: "queued reply"}, Reply: queuedReply}}
	final := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "Final answer", protocol.OutputTargetSlack)
...
func TestSendResponseTerminalIncludesProgressBufferedWhileWaiting(t *testing.T) {
	...
	connector.thinking["run"] = slackThinkingState{
		...
		workflowAgents: []protocol.AgentUpdate{{CallID: "run/agent/000000", PhaseID: phaseID, Label: "worker", Activity: "reading"}},
	...
	agent := protocol.AgentUpdate{CallID: "run/agent/000000", PhaseID: phaseID, Label: "worker", Activity: "verified"}
...
func TestWorkflowAgentChunksRenderOrderedAttributedDeltas(t *testing.T) {
	...
	agents := []protocol.AgentUpdate{
		{CallID: "run/agent/000000", PhaseID: phaseID, Label: "failure-trace", Activity: "read: <https://example.com/report|report>"},
		{CallID: "run/agent/000001", PhaseID: phaseID, Label: "canonical-owner", Activity: "bash: first line\nsecond line"},
	}
	...
	long := protocol.AgentUpdate{CallID: "run/agent/000000", PhaseID: phaseID, Label: "worker", Activity: strings.Repeat("界", 300)}
...
func TestWorkflowAgentUpdatesAccumulateOrderedPhaseDetails(t *testing.T) {
	...
	phase := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "", protocol.OutputTargetSlack)
	...
	for _, update := range []protocol.AgentUpdate{
		...
		msg := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "", protocol.OutputTargetSlack)
...
func TestWorkflowRequestListsLaunchesAndRejectsActiveStack(t *testing.T) {
	...
	assert.Equal(t, protocol.TurnWorkflow, turns[0].Kind)
	assert.Equal(t, "audit", turns[0].Workflow)
	assert.Equal(t, "src/routes", turns[0].WorkflowArgs)
...
func TestWorkflowRequestRejectsBusyConversation(t *testing.T) {
	var ephemeral []url.Values
...
func TestWorkflowUpdateArrivingDuringAppendIsPreserved(t *testing.T) {
	for _, kind := range []string{"agent", "phase"} {
		t.Run(kind, func(t *testing.T) {
			...
			firstAgent := protocol.AgentUpdate{CallID: "run/agent/000000", PhaseID: "run/phase/audit", Label: "worker", Activity: "reading"}
			...
			if kind == "agent" {
				...
				assert.Equal(t, protocol.AgentUpdate{CallID: "run/agent/000000", PhaseID: "run/phase/audit", Label: "worker", Activity: "verified"}, connector.thinking["run"].workflowAgents[0])
			} else {
...
func TestWorkflowFinalizationDuringAppendPreservesLatestPhase(t *testing.T) {
	...
	final := protocol.NewOutboundMessage(protocol.SourceSlack, "thread", "done", protocol.OutputTargetSlack)
	final.TurnID, final.SlackReply, final.Complete, final.WorkflowTerminal = "run", reply, true, protocol.TerminalComplete
...
func TestHandleAppMentionEventRunsOnDemandCronInRootThread(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				return len(runner.runsSnapshot()) == 1
			}, time.Second, time.Millisecond)
...
func TestHandleAppMentionEventStartsGoal(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			turns := waitTurns(t, connector, 1)
			assert.Equal(t, protocol.TurnGoal, turns[0].Kind)
			assert.Equal(t, "social", turns[0].Agent)
			assert.Equal(t, "fix lint", turns[0].Objective)
			assert.Equal(t, 2, turns[0].MaxTurns)
		})
	}
...
func TestHandleAppMentionEventShowsDollarCommandHelp(t *testing.T) {
	...
	for i, text := range []string{"<@U999> $", "<@U999> $wat", "<@U999> $stop", "<@U999> $stop later"} {
		...
		require.GreaterOrEqual(t, len(connector.conv.(*recordingConversationBackend).created), 1)
	}
...
func TestHandleAppMentionEventShowsRootAgentSelector(t *testing.T) {
	...
	assert.NotEmpty(t, connector.conv.(*recordingConversationBackend).created)
	assert.Empty(t, convTurns(connector))
	assert.Empty(t, ephemeral)
...
func TestHandleAppMentionEventStartsNamedAgentWithRootPrompt(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, ok)
			assert.Equal(t, event.TimeStamp, threadTS)

			if tt.forward == nil {
				assert.Equal(t, tt.want, started[0].Text)
			}
...
func TestHandleAppMentionEventPreservesBufferedReplyAcrossRootRedelivery(t *testing.T) {
	...
	require.GreaterOrEqual(t, len(waitTurns(t, connector, 1)), 1)
...
func TestHandleAppMentionEventCleansUpUnregisteredDollarCommandHelp(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "registration failed", inert: true},
		{name: "duplicate event", known: true},
	} {
		t.Run(tt.name, func(t *testing.T) {
			bus := newTestBus()
			defer bus.Close()
...
func TestHandleMessageEventRunsHyphenOnDemandCronInManagedThread(t *testing.T) {
	...
	require.Equal(t, []string{"main cronjob"}, runner.targetsSnapshot())
	assert.Equal(t, []string{protocol.SlackThreadConversationID("C123", event.ThreadTimeStamp)}, connector.conv.(*recordingConversationBackend).agentReadSnapshot())
	assert.Empty(t, router.cronRegistrationsSnapshot())
}
...
func TestHandleMessageEventRunsOnDemandCronStartsNewPair(t *testing.T) {
	router := newThreadRouterStub()
	router.submitHandled = true
	runner := newOneOffCronjobLoaderStub()
...
func TestHandleMessageEventRunsOnDemandCronWithoutInvokingThreadUI(t *testing.T) {
	runner := newOneOffCronjobLoaderStub()
	runner.loaded = protocol.OneOffCronjob{Agent: "cron", Prompt: "daily prompt", RelativePath: "cron/daily.md", TextChannel: "#ops"}
	router := newThreadRouterStub()
	router.submitHandled = true
...
func TestHandleOnDemandCronRequestDoesNotRegisterInvokingThread(t *testing.T) {
	runner := newOneOffCronjobLoaderStub()
...
func TestHandleMessageEventSkipsThreadReplyWhenPrepareFails(t *testing.T) {
	...
	assert.Empty(t, router.repliesSnapshot())
	assert.Equal(t, []string{protocol.SlackThreadConversationID("C123", "171234.5678")}, connector.conv.(*recordingConversationBackend).agentReadSnapshot())
}
...
func TestResolveManagedThreadTSFallsBackToSlackReactions(t *testing.T) {
	...
	router := newThreadRouterStub()
	connector := newTestConnectorWithOptions(server.URL, bus, nil, router, nil)
	connector.conv.(*recordingConversationBackend).known = map[string]string{
		protocol.SlackThreadConversationID("C123", "171234.5678"): "social",
	}
...
func TestHandleReactionAddedEventStopsReplyThread(t *testing.T) {
	for _, reaction := range []string{slackGoalStopSignReaction, slackGoalStopButtonReaction} {
		t.Run(reaction, func(t *testing.T) {
			...
			progress := protocol.NewOutboundMessage(protocol.SourceSystem, protocol.SlackThreadConversationID("C123", "171234.5678"), "", protocol.OutputTargetSlack)
...
func TestHandleReactionAddedEventStopsExternalMCPResponseConversation(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, protocol.TurnCancel, turns[0].Kind)
	assert.Equal(t, "external_mcp:customer:private", turns[0].ID)
}
...
func TestHandleReactionAddedEventEnvelopeStopsDropsEnqueueWithoutStoppingTurn(t *testing.T) {
	...
	assert.Empty(t, laterWork(connector))
	assert.Contains(t, reactions, "/reactions.remove "+slackEnvelopeReaction+" 111.2")
	assert.Empty(t, convTurns(connector))
	assert.Len(t, posted, postedBefore)
}
...
func TestHandleReactionAddedEventThinkingStillStopsWithQueuedItem(t *testing.T) {
	...
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, protocol.TurnCancel, turns[0].Kind)
	require.Len(t, laterWork(connector), 1)
	assert.Equal(t, "q1", laterWork(connector)[0].ID)
}
...
func TestHandleReactionAddedEventMCPEmptySlackTSIsNotCancelled(t *testing.T) {
	...
	require.Len(t, laterWork(connector), 1)
	assert.Equal(t, "mcp1", laterWork(connector)[0].ID)
	turns := waitTurns(t, connector, 1)
	assert.Equal(t, protocol.TurnCancel, turns[0].Kind)
}
...
func TestHandleReactionAddedEventFastUpConvertsEnqueueToSteer(t *testing.T) {
	for _, reaction := range []string{slackFastUpButtonReaction, slackBlackUpPointingDoubleTriangleReaction, slackArrowDoubleUpReaction} {
		t.Run(reaction, func(t *testing.T) {
			...
			assert.Contains(t, reactions, "/reactions.add "+slackBufferedReaction+" 111.2")
			turns := waitTurns(t, connector, 1)
			require.Equal(t, protocol.TurnSteer, turns[0].Kind)
			assert.Equal(t, "later", turns[0].Text)
			assert.Len(t, posted, postedBefore)
...
func TestHandleBroadcastDropsEmptyBroadcast(t *testing.T) {
	connector := newTestConnector("http://slack.test")

	acknowledgement := connector.HandleBroadcast(t.Context(), &protocol.Broadcast{})
	require.Equal(t, protocol.BroadcastDropped, acknowledgement.Status)
}

func TestHandleBroadcastDropsWebTarget(t *testing.T) {
	connector := newTestConnector("http://slack.test")
	message := protocol.NewOutboundMessage(protocol.SourceWeb, protocol.WebSessionConversationID("ops"), "hello", protocol.OutputTargetWeb)
	acknowledgement := connector.HandleBroadcast(t.Context(), &protocol.Broadcast{Message: message, Delivery: message})
	require.Equal(t, protocol.BroadcastDropped, acknowledgement.Status)
}

func TestHandleBroadcastCronjobFailure(t *testing.T) {
	connector := newTestConnector("http://slack.test")
	delivery := protocol.NewOutboundMessage(protocol.SourceSystem, "conversation", "cron")
	delivery.Complete = true
	delivery.Cronjob = &protocol.CronjobMessage{RelativePath: "daily.md", Agent: "main", RanAt: "now"}
	...
	require.Equal(t, protocol.BroadcastFailed, ack.Status)
	require.Error(t, ack.Err)
	require.Error(t, delivery.WaitDelivered(t.Context()))
}

func TestHandleBroadcastSendResponseSuccess(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/chat.postMessage", "/chat.update", "/reactions.add":
	...
	connector := newTestConnector(server.URL)
	message := protocol.NewOutboundMessage(protocol.SourceSlack, "conversation", "hello")
	message.Complete = true
	message.Targets = []protocol.OutputTarget{protocol.OutputTargetSlack}
	message.SlackReply = &protocol.SlackReplyTarget{ChannelID: "C123", MessageTS: "1.1", ThreadTS: "1.1"}
	ack := connector.HandleBroadcast(t.Context(), &protocol.Broadcast{Message: message, Delivery: message})
...
func newTestConnectorWithOptions(apiURL string, bus *testBus, channels []config.SlackChannelConfig, router *threadRouterStub, runner oneOffCronjobRunner) *Connector {
	...
	rec := &recordingConversationBackend{turns: make(chan protocol.TurnRequest, 64)}
	if router != nil {
		rec.queue = slices.Clone(router.queue)
		rec.busy = router.busy
		rec.scheduled = router.scheduled
		rec.workflows = slices.Clone(router.workflows)

		rec.agentErr = router.errPrepare
		if router.submitHandled || router.prepareHandled || router.threadAgentHandled {
			rec.agent = router.threadAgent
			if rec.agent == "" {
				rec.agent = "social"
			}
		}
...
func setLaterWork(c *Connector, items []protocol.ThreadQueueItem) {
	c.conv.(*recordingConversationBackend).queue = items
}

func knownY(c *Connector, agent string) {
	c.conv.(*recordingConversationBackend).agent = agent
}

func convTurns(c *Connector) []protocol.TurnRequest {
	return c.conv.(*recordingConversationBackend).turnSnapshot()
}

func laterWork(c *Connector) []protocol.ThreadQueueItem {
	return slices.Clone(c.conv.(*recordingConversationBackend).queue)
}

func waitTurns(t *testing.T, c *Connector, n int) []protocol.TurnRequest {
	t.Helper()

	var turns []protocol.TurnRequest
...
type threadRouterStub struct {
	...
	errQueue            error
	errPick             error
	queuedPicks         []cronThreadRegistration
	registerExisting    bool
...
func (s *threadRouterStub) RegisterCronThread(_ context.Context, target protocol.TextConversationTarget, agent string) error {
	s.mu.Lock()
...
func (s *threadRouterStub) ThreadAgent(target protocol.TextConversationTarget) (agent string, handled bool, err error) {
	...
	if !handled {
		for _, reg := range s.cronRegistrations {
			if reg.channelID == target.ChannelID && reg.threadTS == target.ThreadID {
				handled = true
...
func (s *threadRouterStub) SubmitWhenActive(ctx context.Context, target protocol.TextConversationTarget, inbound *protocol.InboundMessage, activation protocol.ActivationHook) (bool, error) {
...
func (s *threadRouterStub) ThreadQueueItems(context.Context, protocol.TextConversationTarget) ([]protocol.ThreadQueueItem, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *threadRouterStub) ReorderThreadQueueItems(_ context.Context, _ protocol.TextConversationTarget, ids []string) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	byID := make(map[string]protocol.ThreadQueueItem, len(s.queue))
	for i := range s.queue {
...
func (s *threadRouterStub) PickQueuedWork(_ context.Context, target protocol.TextConversationTarget) error {
	s.mu.Lock()
	s.queuedPicks = append(s.queuedPicks, cronThreadRegistration{channelID: target.ChannelID, threadTS: target.ThreadID})
	errPick := s.errPick
	s.mu.Unlock()
