...
func TestSideAskEndDoesNotPromoteBufferedFollowUp(t *testing.T) {
	...
	followUp := &events.SlackReplyTarget{ChannelID: "C123", MessageTS: "222.334", ThreadTS: "111.222"}
	content := events.InboundContent{Text: "thread follow-up"}
...
func TestSideAskUsesChosenAgentWithoutChangingThreadOwner(t *testing.T) {
	...
	agent, handled, err := router.ThreadAgent(events.TextConversationTarget{ChannelID: "C123", ThreadID: "111.222"})
...
func TestSideAskThinkingUpdatesModalWithoutThreadPosts(t *testing.T) {
	recorder := newSideAskModalRecorder(t)
	connector := newTestConnector(recorder.URL)
	connector.sideAskHost = scriptedSideAskHost{run: func(ctx context.Context, req harnessbridge.SideAskRequest) error {
...
func TestSideAskRunnerErrorUpdatesCloseOnlyErrorView(t *testing.T) {
	recorder := newSideAskModalRecorder(t)
	connector := newTestConnector(recorder.URL)
	connector.sideAskHost = scriptedSideAskHost{run: func(context.Context, harnessbridge.SideAskRequest) error {
...
type scriptedSideAskHost struct {
	run func(context.Context, harnessbridge.SideAskRequest) error
}

func (s scriptedSideAskHost) Run(ctx context.Context, req harnessbridge.SideAskRequest) error {
