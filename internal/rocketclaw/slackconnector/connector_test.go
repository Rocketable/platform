...
func TestStartNewThreadRootPostsMessageAndPermalink(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	result, err := connector.StartNewThreadRoot(context.Background(), &events.StartNewThreadRequest{
		Title:      "Child",
		Prompt:     "Do the work",
		SlackReply: &events.SlackReplyTarget{ChannelID: "C123", MessageTS: "111.222", ThreadTS: "111.222"},
	})
	require.NoError(t, err)
	assert.Equal(t, events.TextConversationTarget{ChannelID: "C123", MessageID: "999.000", ThreadID: "999.000"}, result.Target)
...
func TestAskUserQuestionUsesUniqueSlackButtonActionIDs(t *testing.T) {
	...
	type askResult struct {
		answer events.AskUserQuestionAnswer
		err    error
	}
	...
	go func() {
		answer, err := connector.AskUserQuestion(context.Background(), &events.AskUserQuestionRequest{
			ID:       "question-123",
			Question: "Choose one.",
	...
	require.True(t, connector.completeQuestion(context.Background(), "question-123", events.AskUserQuestionAnswer{Selected: []string{"approve"}, Source: events.SourceSlack}))

	result := <-done
	require.NoError(t, result.err)
	assert.Equal(t, events.AskUserQuestionAnswer{Selected: []string{"approve"}, Source: events.SourceSlack}, result.answer)
...
func TestAskUserQuestionAlwaysRendersSlackCustomButton(t *testing.T) {
	...
	require.True(t, connector.completeQuestion(context.Background(), "question-123", events.AskUserQuestionAnswer{Custom: "ok", Source: events.SourceSlack}))
...
func TestAskUserQuestionIncludesDetailsInPost(t *testing.T) {
	...
	go func() {
		_, err := connector.AskUserQuestion(context.Background(), &events.AskUserQuestionRequest{
			ID:         "question-details",
			Question:   "Choose?",
	...
	require.True(t, connector.completeQuestion(context.Background(), "question-details", events.AskUserQuestionAnswer{Custom: "x", Source: events.SourceSlack}))
	require.NoError(t, <-done)
...
func TestAskUserQuestionMultipleSelectRendersMultiSelect(t *testing.T) {
	...
	require.True(t, connector.completeQuestion(context.Background(), "question-multi", events.AskUserQuestionAnswer{Selected: []string{"a", "b"}, Source: events.SourceSlack}))
	require.NoError(t, <-done)
...
func TestHandleBroadcastRelayCleanup(t *testing.T) {
	connector := newTestConnector("http://slack.test")
	reply := make(chan events.BroadcastReply, 1)
	ack := connector.HandleBroadcast(t.Context(), &events.Broadcast{
		RelayCleanup:  &events.InboundMessage{SlackReply: &events.SlackReplyTarget{ChannelID: "C1", MessageTS: "1.1", ThreadTS: "1.1"}},
		RelayResponse: reply,
	})
	assert.Equal(t, events.BroadcastHandled, ack.Status)
...
func TestCompleteQuestionUnknownIDReturnsFalse(t *testing.T) {
	connector := newTestConnector("http://slack.test")
	assert.False(t, connector.completeQuestion(context.Background(), "missing", events.AskUserQuestionAnswer{}))
}
...
func TestDeleteQuestionMessageLogsFailures(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	connector.deleteQuestionMessage(context.Background(), events.TextConversationTarget{ChannelID: "C123", MessageID: "1.2"})
}

func TestSetMCPAttachmentOnlyResponseText(t *testing.T) {
	msg := events.NewOutboundMessage(events.SourceExternalMCP, "conversation", "")
	msg.Complete = true
	msg.ExternalConversationID = "public-1"
	msg.Attachments = []events.OutboundAttachment{{Name: "a.txt"}}
	setMCPAttachmentOnlyResponseText(msg)
	assert.NotEmpty(t, msg.Text)

	empty := events.NewOutboundMessage(events.SourceExternalMCP, "conversation", "")
	empty.Complete = true
	empty.ExternalConversationID = "public-1"
	empty.Attachments = []events.OutboundAttachment{{}}
...
func TestAskUserQuestionCancelDeletesUnansweredQuestion(t *testing.T) {
	...
	go func() {
		_, err := connector.AskUserQuestion(ctx, &events.AskUserQuestionRequest{
			ID:         "question-cancel",
			Question:   "Choose?",
			SlackReply: &events.SlackReplyTarget{ChannelID: "C123", MessageTS: "111.222", ThreadTS: "111.222"},
		})
...
func TestHandleInteractiveAnswersQuestionBySlackBlockID(t *testing.T) {
	...
	answerCh := make(chan events.AskUserQuestionAnswer, 1)
	connector.questions["question-123"] = &slackPendingQuestion{
		target: events.TextConversationTarget{ChannelID: "C123", MessageID: "555.666", ThreadID: "111.222"},
		ch:     answerCh,
	}
	...
	assert.Equal(t, events.AskUserQuestionAnswer{Selected: []string{"defer"}, Source: events.SourceSlack}, <-answerCh)
	assert.Equal(t, "C123", deleted.Get("channel"))
	assert.Equal(t, "555.666", deleted.Get("ts"))
}
...
func TestHandleInteractiveCustomQuestionSubmissionAnswersQuestion(t *testing.T) {
	...
	assert.Equal(t, events.AskUserQuestionAnswer{Custom: "custom answer", Source: events.SourceSlack}, <-answerCh)
	assert.Equal(t, "C123", deleted.Get("channel"))
	assert.Equal(t, "555.666", deleted.Get("ts"))
}
...
func TestHandleMessageEventBuffersSlackMessagesWhileActive(t *testing.T) {
	...
	connector.mu.Lock()
	pending := slices.Clone(connector.stacks[slackThreadStackKey(&events.SlackReplyTarget{ChannelID: "C123", ThreadTS: "111.0"})])
	connector.mu.Unlock()
...
func TestDrainSteersRemovesHourglassAndReturnsSendOrder(t *testing.T) {
	...
	assert.Empty(t, router.queueSnapshot())
	connector.mu.Lock()
	pending, active := connector.stacks[slackThreadStackKey(&events.SlackReplyTarget{ChannelID: "C123", ThreadTS: "111.0"})]
	connector.mu.Unlock()
...
func TestRestorePendingSteersInjectsAtToolBoundary(t *testing.T) {
	...
	conversationID := harnessbridge.SlackThreadConversationID("C123", "111.0")
	connector.RestorePendingSteers(conversationID, []harnessbridge.PendingSteer{
...
func TestPendingSteersPersistOnBufferAndClearOnStop(t *testing.T) {
	var persisted [][]harnessbridge.PendingSteer
	...
	router.submitHandled = true
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, router, nil)
	connector.SetPendingSteersSink(harnessbridge.PendingSteersSink{Set: func(_ string, steers []harnessbridge.PendingSteer) error {
		persisted = append(persisted, append([]harnessbridge.PendingSteer(nil), steers...))
		return nil
	}})
...
func TestActivateEnqueuePostsConsumeCardThenPlaceholders(t *testing.T) {
	...
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, newThreadRouterStub(), nil)
	inbound := events.NewInboundMessage(events.SourceSlack, events.InboundKindPrompt, "enqueued_message", "write the changelog", false)
	inbound.SlackReply = &events.SlackReplyTarget{ChannelID: "C123", MessageTS: "111.2", ThreadTS: "111.0"}
	require.NoError(t, connector.ActivateEnqueue(t.Context(), &harnessbridge.ThreadQueueItem{ID: "q1", SlackChannel: "C123", SlackTS: "111.2"}, inbound))
...
func TestDiscardPendingSteersAddsInterruption(t *testing.T) {
	...
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, newThreadRouterStub(), nil)
	connector.DiscardPendingSteers(t.Context(), []harnessbridge.PendingSteer{{SlackChannel: "C123", SlackTS: "111.2", SlackThreadTS: "111.0"}})
	assert.Contains(t, reactions, "/reactions.remove "+slackBufferedReaction+" 111.2")
...
func TestHandleMessageEventIdleEnqueueStartsNow(t *testing.T) {
	...
	_, active := connector.stacks[slackThreadStackKey(&events.SlackReplyTarget{ChannelID: "C123", ThreadTS: "111.0"})]
	assert.True(t, active)
}
...
func TestHandleMessageEventIdleEnqueueStartsEvenIfQueueNonempty(t *testing.T) {
	...
	router := newThreadRouterStub()
	router.submitHandled = true
	router.queue = []harnessbridge.ThreadQueueItem{{ID: "existing", Message: "older", Position: 0}}
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, router, nil)
...
func TestHandleMessageEventWorkflowAllowedWhenQueueExistsWithoutLiveTurn(t *testing.T) {
	...
	router := newThreadRouterStub()
	router.prepareHandled = true
	router.queue = []harnessbridge.ThreadQueueItem{{ID: "q1", Message: "later"}}
	connector := newTestConnectorWithOptions(server.URL, newTestBus(), nil, router, nil)
...
func TestHandleInteractiveQueueRemoveByAnotherAllowedUser(t *testing.T) {
	...
	callback := slackQueueCallback("555.1", "U456", slackQueueRemoveActionID, "q1")
	connector.handleInteractive(t.Context(), socketmode.Event{Data: callback})
...
func TestHandleInteractiveQueueCancelScheduled(t *testing.T) {
	...
	callback := slackQueueCallback("555.1", "U123", slackQueueCancelScheduledActionID, "s1")
	connector.handleInteractive(t.Context(), socketmode.Event{Data: callback})
...
func TestHandleInteractiveQueueResetScheduledLeavesEnqueue(t *testing.T) {
	...
	callback := slackQueueCallback("555.1", "U123", slackQueueResetScheduledActionID, "")
	connector.handleInteractive(t.Context(), socketmode.Event{Data: callback})
...
func TestHandleInteractiveQueueClickUnauthorizedIsIgnored(t *testing.T) {
	...
	router := newThreadRouterStub()
	router.prepareHandled = true
	router.queue = []harnessbridge.ThreadQueueItem{{ID: "q1", Message: "keep"}}
	...
	callback := slackQueueCallback(posted[0].Get("ts"), "U999", slackQueueRemoveActionID, "q1")
	connector.handleInteractive(t.Context(), socketmode.Event{Data: callback})
...
func TestHandleInteractiveQueueStaleRowRefreshesCard(t *testing.T) {
	...
	callback := slackQueueCallback("555.1", "U123", slackQueueRemoveActionID, "gone")
	connector.handleInteractive(t.Context(), socketmode.Event{Data: callback})
...
func slackQueueCallback(messageTS, userID, actionID, itemID string) slack.InteractionCallback {
...
func TestSendResponseTerminalIncludesProgressBufferedWhileWaiting(t *testing.T) {
	...
	connector.thinking["run"] = slackThinkingState{
		...
		workflowAgents: []workflow.AgentUpdate{{CallID: "run/agent/000000", PhaseID: phaseID, Label: "worker", Activity: "reading"}},
...
func TestWorkflowAgentChunksRenderOrderedAttributedDeltas(t *testing.T) {
	...
	chunks := slackWorkflowAgentChunks([]workflow.AgentUpdate{long}, phases, nil)
	require.Len(t, chunks, 1)
	task := chunks[0].(slack.TaskUpdateChunk)
	assert.LessOrEqual(t, len([]rune(task.Details)), 256)

	historyChunks := slackWorkflowHistoryChunks(phases, nil, []workflow.AgentUpdate{long})
...
func TestSlackTaskStatusMapsWorkflowStatuses(t *testing.T) {
	for _, tt := range []struct {
		...
		{status: workflow.PhasePending, want: slack.TaskCardStatusPending},
		{status: workflow.PhaseInProgress, want: slack.TaskCardStatusInProgress},
		{status: workflow.PhaseComplete, want: slack.TaskCardStatusComplete},
		{status: workflow.PhaseError, want: slack.TaskCardStatusError},
		{status: workflow.PhaseSkipped, want: slack.TaskCardStatusComplete},
	} {
...
func TestWorkflowAgentDetailsRenderUnderOwningPhase(t *testing.T) {
	...
	phase := events.NewOutboundMessage(events.SourceSlack, "thread", "", events.OutputTargetSlack)
	phase.TurnID, phase.SlackReply = "run", reply
	phase.WorkflowPhase = &workflow.PhaseUpdate{PhaseID: "run/phase/000001/investigate", Name: "investigate", Status: workflow.PhaseInProgress, Scheduled: 2, Running: 2}
	require.NoError(t, connector.SendResponse(t.Context(), phase))
	...
	for _, kind := range []string{"agent", "phase"} {
		t.Run(kind, func(t *testing.T) {
			...
			if kind == "agent" {
				...
				assert.Equal(t, workflow.AgentUpdate{CallID: "run/agent/000000", PhaseID: "run/phase/audit", Label: "worker", Activity: "verified"}, connector.thinking["run"].workflowAgents[0])
			} else {
...
func TestHandleBroadcastDropsEmptyBroadcast(t *testing.T) {
	...
	acknowledgement := connector.HandleBroadcast(t.Context(), &events.Broadcast{})
...
func TestHandleBroadcastCronjobFailure(t *testing.T) {
	...
	delivery.Cronjob = &events.CronjobMessage{RelativePath: "daily.md", Agent: "main", RanAt: "now"}
	delivery.SlackReply = &events.SlackReplyTarget{ChannelID: "C123"}
	ack := connector.HandleBroadcast(t.Context(), &events.Broadcast{Message: delivery, Delivery: delivery})
	require.Equal(t, events.BroadcastFailed, ack.Status)
...
func TestHandleBroadcastSendResponseSuccess(t *testing.T) {
	...
	connector := newTestConnector(server.URL)
	message := events.NewOutboundMessage(events.SourceSlack, "conversation", "hello")
	message.Complete = true
	message.Targets = []events.OutputTarget{events.OutputTargetSlack}
	message.SlackReply = &events.SlackReplyTarget{ChannelID: "C123", MessageTS: "1.1", ThreadTS: "1.1"}
	ack := connector.HandleBroadcast(t.Context(), &events.Broadcast{Message: message, Delivery: message})
	require.Equal(t, events.BroadcastHandled, ack.Status)
	require.NoError(t, message.WaitDelivered(t.Context()))
}

func newTestConnectorWithOptions(apiURL string, bus *testBus, channels []config.SlackChannelConfig, router harnessbridge.PrimaryTextRouter, runner oneOffCronjobRunner) *Connector {
	logger := testLogger()
...
func readOneOutbound(t *testing.T, bus *testBus) *events.OutboundMessage {
	t.Helper()
...
func newThreadRouterStub() *threadRouterStub {
	return &threadRouterStub{workflowReserved: true, scheduled: map[string]harnessbridge.ScheduledMessageState{}}
}
...
func (s *threadRouterStub) StashThreadQueueItem(_ context.Context, target events.TextConversationTarget, item *harnessbridge.ThreadQueueItem) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	item.ConversationID = harnessbridge.SlackThreadConversationID(target.ChannelID, target.ThreadID)
...
func (s *threadRouterStub) ThreadQueueItems(context.Context, events.TextConversationTarget) ([]harnessbridge.ThreadQueueItem, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *threadRouterStub) ScheduledMessages(context.Context, events.TextConversationTarget) (map[string]harnessbridge.ScheduledMessageState, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
