...
| `agents/`, `skills/`, `scripts/` | User-overridable workspace overlays for agent, skill, and script assets. Agent files may declare `guardrail: <agent-name>` to reference another loaded agent as a per-target task guardrail. Changes require restart to affect running RocketCode definitions. Local workspace overlays are applied after embedded assets and configured git overlays. Startup exposes effective runtime script files from `<runtime-dir>/scripts/` as symlinks under workspace `scripts/`, preserving existing regular workspace script files. Startup removes existing generated script symlinks before scanning local workspace overlays so stale runtime outputs are never treated as local overlay sources. When `<workspace>/.git` is a directory, startup maintains a managed block in `.git/info/exclude` for the generated RocketClaw workspace script symlinks so they do not pollute git status. |
...
- Before effective runtime assets are built, startup removes workspace `scripts/` symlinks that resolve into `.rocketclaw/` or `.femtoclaw/` so stale generated links cannot be copied back as local overlay input. After effective runtime assets are built, startup recreates symlinks for files from the selected `<runtime-dir>/scripts/`. Regular workspace script files and symlinks to other locations are preserved.
- When `<workspace>/.git` is a directory, startup updates only `.git/info/exclude` with a managed block for the generated RocketClaw workspace script symlink paths. Startup preserves unrelated exclude content, replaces any previous RocketClaw managed block, removes the block when there are no generated script symlinks, and does not touch `scripts/.gitignore` or workspace `.gitignore`. Non-git workspaces and git worktrees whose `.git` is not a directory are ignored by this behavior.
