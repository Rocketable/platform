...
| --- | --- |
| `rocketclaw.json` | Main runtime config. Relative `workspace` resolves relative to the config file. At least one of Slack or external MCP must be enabled. Slack is the primary text connector. Optional `overlays` entries name git repositories whose `agents/`, `skills/`, `cron/`, and `scripts/` trees are applied during startup. Optional top-level `auto_approver_model` selects the embedded RocketCode automatic permission reviewer model for bare `auto` permission rules. Optional top-level `seed_compaction_model` selects the first-party OpenAI Responses model RocketClaw uses for inherited-context seed replay compaction. RocketCode model requests use first-party OpenAI Responses through the configured OpenAI RocketCode auth path. |
| `femtoclaw.json` | Legacy runtime config. If present, startup and operational commands load it instead of `rocketclaw.json` and use `.femtoclaw/` as the generated runtime directory. It supports the same optional `overlays` entries, `auto_approver_model` setting, and `seed_compaction_model` setting as `rocketclaw.json`. |
...
- Empty or omitted `seed_compaction_model` preserves source-model seed compaction selection: use the source checkpoint or latest source session entry model when present, otherwise `gpt-5.5`. Non-empty values select the first-party OpenAI Responses seed compaction model and must be unprefixed first-party OpenAI model IDs or legacy `openai/<model>` aliases normalized by RocketCode-style validation; other provider-qualified values are config validation errors.
