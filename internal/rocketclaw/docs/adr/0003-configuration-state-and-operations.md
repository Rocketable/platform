...
| --- | --- |
| `rocketclaw.json` | Main runtime config. Relative `workspace` resolves relative to the config file. Slack is always active. The Slack object contains `bot_token`, `app_token`, and `channels`. Each channel has a non-empty ordered `agents` list and non-empty `allowed_user_ids`. Every External MCP call names one of these channels. Its private MCP agent may be any loaded agent. The managed Slack session starts with the channel's first agent. Optional `overlays` entries name git repositories whose `agents/`, `skills/`, `cron/`, and `scripts/` trees are applied during startup. Optional top-level `models` maps placeholder names to deployment model names. Optional top-level `auto_approver_model` selects the embedded RocketCode automatic permission reviewer model for bare `auto` permission rules. RocketCode model requests use first-party OpenAI Responses through the configured OpenAI RocketCode auth path. |
...
| `<runtime-dir>/state.sqlite3` | Stores private MCP histories, managed Slack histories, Slack routing, and the link from each public MCP conversation ID to both histories. That link also stores the fixed private agent and Slack channel. The database also stores unfinished turns for restart recovery, scheduled messages, scheduled cron state, goals, restart notices, and first prompts for runtime-created conversations. A private MCP entry and its managed copy are saved in one database transaction. A crash therefore cannot save the private entry without its managed copy. Cleanup uses conversation IDs, not agent names. ADR 0005 defines how RocketClaw opens this database. |
...
- 2026-07-16: Added separate private MCP and managed Slack histories, links to both histories, independent agents, and one-transaction MCP-to-managed history copying without compactions.
