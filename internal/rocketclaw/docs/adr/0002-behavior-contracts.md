...
- Slack binding: Slack stacked messages use Slack buffering semantics; normal Slack-visible assistant turns reserve a thinking placeholder (`_Thinking..._`) plus answer placeholder (`\u200B`), and once reserved the pair must be consumed through normal Slack final-response machinery. Slack thinking placeholder updates render accumulated RocketCode progress as a quote block with the newest progress line first while preserving chronological accumulation internally.
...
- Discord Text binding: Discord-visible assistant turns use a visible progress message as the progress surface. Discord guild-channel conversations may use guild threads; Discord DM conversations are managed text conversations without guild-thread semantics.
