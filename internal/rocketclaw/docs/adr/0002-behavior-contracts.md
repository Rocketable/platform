...
- Managed text conversations and response-rooted text conversations remain isolated from main until summarized. When those conversations start with inherited main-session context, RocketClaw seeds them from the latest available compaction boundary when one exists; if no compaction boundary exists, RocketClaw may compact the selected main-session history first. This same seeding flow must work for OpenAI, OpenAI-compatible, and Anthropic model selections. Provider-specific compaction and replay mechanics are implementation details hidden behind the same product, operational, and internal capability. Shared interfaces and construction paths must require each provider implementation to participate in the same seeding, replay projection, and continuation contracts rather than leaving provider-specific unsupported branches. Seed replay uses the same provider/mode projection semantics as RocketCode runtime replay, including target-specific native compaction, user checkpoint fallback, and encrypted reasoning omission for incompatible targets.
