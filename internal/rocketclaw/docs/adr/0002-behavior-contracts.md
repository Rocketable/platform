...
- Social-mode channel configuration uses canonical `agents`, an ordered list of allowed agent names. The first configured agent is the default agent for newly started social-mode managed conversations. Empty or duplicate `agents` entries are normalized out before routing.
- External MCP conversations are isolated by external conversation ID; omitted ID starts a new isolated conversation. For a new external MCP conversation, the request must explicitly name a loaded agent, and that agent becomes the persisted conversation agent. For an existing external MCP conversation ID, the persisted agent is authoritative; if a request supplies a different agent, RocketClaw logs the mismatch and continues the turn with the persisted agent instead of rejecting the request or changing the persisted agent.
...
- External MCP active-turn checkpoints use the private durable RocketClaw/RocketCode conversation ID associated with the public external conversation ID. Recovery may use a persisted Slack relay or thread alias when one exists, but it must not promise to complete the old blocking HTTP/MCP `session_prompt` request after process restart because that request waited on process-local state. A later `session_prompt` for the same public external conversation ID must resolve to the same private conversation, queue behind any active recovered turn, and wait only for its own later turn result. Its Slack relay must not visibly overtake the recovered turn; relay posting belongs on the same per-conversation queued execution path as the prompt it represents.
...
- The primary text output target is configured as Slack DM when Slack is enabled.
...
- Slack tool-created root messages are concise human-visible setup messages, not inherited context carriers. Their visible text is exactly:
...
- Authorized social-mode managed conversation replies may switch that conversation's persisted agent by sending exactly `🎛 agent-name` as the whole message, where `agent-name` is one of the configured `agents` entries for the social-mode channel that owns the conversation. Authorized managed conversation replies that send exactly bare `🎛` as the whole message must receive a Slack-native agent selector in the managed conversation, populated from the owning social-mode channel's current configured `agents` entries. A selector choice may switch the persisted agent only when the interacting Slack user is the same authorized user who sent the bare `🎛` message, and the selected agent is still configured for that social-mode channel at interaction time. Agent-switch and selector control messages and interactions must not create placeholders, reactions, prompt context, attachment processing, or thread-router submissions as model input. Invalid agent names and invalid selector choices must be rejected without changing the persisted conversation agent. Slack rejects invalid or failed switches with private ephemeral feedback and acknowledges successful switches with a normal visible thread reply.
- Main-session stop controls for the enabled primary text connector must interrupt the active main turn when the configured human sends `🛑` or `⏹️` as an exact top-level DM message, or adds a connector stop reaction to the main-session turn surface. Slack stop reactions are `:octagonal_sign:` and `:stop_button:`. Main-session stop controls must clear queued main bridge work and connector-buffered main work, send no stop text, and add only a persistent `❗` reaction to the original turn-start message for the interrupted active main turn.
...
- Every completed scheduled cron publishes one silent run summary to `main`. The `rocketclaw_i_want_human_partner_to_see_this` argument is separate extra output for the human: an empty string sends no connector output, and a non-empty string is submitted to the cron text delivery sink. With Slack enabled, non-empty extra output starts or uses a managed conversation through Slack: if `channel` is present, that value is the target; otherwise the target is Slack's configured default DM room. Non-empty extra cron output must not be sent as main-session verbatim output. With no enabled primary text connector, the inert delivery sink accepts the output and the silent main summary still runs. Replies and summaries for cron-created connector conversations follow existing connector gates.
...
- The `ask_user_question` tool is a persistent-bridge RocketClaw tool visible only during human-initiated turns that have a native Slack answer path: `SourceSlack`, `Human == true`, and `SlackReply != nil`. The tool is available in main-session text turns and managed text conversation turns, including human goal kickoff and human re-steering turns when the originating turn otherwise qualifies. The tool is not visible for cron/raw runs, External MCP turns, scheduled-message turns, system/automation turns, automatic goal continuations, or restart recovery continuations.
