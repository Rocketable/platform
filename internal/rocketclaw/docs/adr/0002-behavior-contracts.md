...
- Slack binding: Slack stacked messages use Slack buffering semantics. A human-originated Slack turn with an identified Slack user and workspace starts the thinking message with `chat.startStream`, then reserves a separate answer placeholder (`\u200B`) with `chat.postMessage`. Thinking progress is appended as Slack Markdown with `chat.appendStream`, and `chat.stopStream` ends the thinking stream after the final answer consumes the answer placeholder. This order is fixed so thinking and answer output may arrive internally out of order without changing their Slack message order. A turn without an identified Slack recipient reserves the existing thinking or goal-progress message plus answer placeholder (`\u200B`). Its thinking and goal-progress updates render one `task_card` Block without a separate label section. The card uses the turn ID as `task_id`, `_Thinking..._` or the goal-progress phrase as its title, and `in_progress` status while the turn runs. Every activity appears from oldest to newest in `details`, with one rich-text section per activity. Slack link markup in an activity uses rich-text link elements so links render; other activity text remains literal. On successful turn completion, RocketClaw keeps the thinking message, sets the card title to `Complete`, and changes the card status to `complete`. The answer remains a separate message in both paths. The fallback message text keeps its existing ordering. No output, sources, feedback, or action controls are added.
