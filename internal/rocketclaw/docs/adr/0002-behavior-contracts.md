...
- Authorized social-mode managed conversation replies may switch that conversation's persisted agent by sending exactly `🎛 agent-name` as the whole message, where `agent-name` is one of the configured `agents` entries for the social-mode channel that owns the conversation. Slack and Discord Text authorized managed conversation replies that send exactly bare `🎛` as the whole message must receive a connector-native agent selector in the managed conversation, populated from the owning social-mode channel's current configured `agents` entries. A selector choice may switch the persisted agent only when the interacting connector user is the same authorized user who sent the bare `🎛` message, and the selected agent is still configured for that social-mode channel at interaction time. Agent-switch and selector control messages and interactions must not create placeholders, reactions, prompt context, attachment processing, or thread-router submissions as model input. Invalid agent names and invalid selector choices must be rejected without changing the persisted conversation agent. Slack rejects invalid or failed switches with private ephemeral feedback and acknowledges successful switches with a normal visible thread reply. Discord Text rejects or acknowledges with a normal visible channel reply.
