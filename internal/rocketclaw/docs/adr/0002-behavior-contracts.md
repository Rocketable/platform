...
- Each persistent conversation owns one FIFO input sequence. Accepted human, scheduled, goal-continuation, recovery, and External MCP work for the same conversation runs in that sequence; independent conversations may proceed concurrently.
...
- External MCP conversations are isolated by external conversation ID; omitted ID starts a new isolated conversation. Every turn must name a configured Slack channel. The first turn creates a fresh Slack thread and binds the external conversation ID to that thread and its persistent conversation history. A later turn must name the same persisted channel and is rejected when the channel is missing or different. Authorized Slack replies and MCP turns in the bound thread share one FIFO conversation history, while different external conversation IDs remain isolated even in the same channel. For a new conversation, the request must explicitly name a loaded agent, and that agent becomes authoritative; if an existing conversation request supplies a different agent, RocketClaw logs the mismatch and continues with the persisted agent.
- External MCP prompt relays must deliver MCP-provided attachments as part of the same visible native Slack message as the relay text. If the relay text is blank and attachments exist, the relay text is `events.AttachmentNamesSpeech(attachments)` when names exist and `Attached files.` otherwise.
...
- External MCP active-turn checkpoints use the durable Slack-bound conversation ID associated with the public external conversation ID. Recovery must not promise to complete the old blocking HTTP/MCP `session_prompt` request after process restart because that request waited on process-local state. A later `session_prompt` for the same public external conversation ID and persisted channel resolves to the same conversation, queues behind any recovered turn, and waits only for its own result. Its Slack relay must not visibly overtake the recovered turn; relay posting belongs on the same per-conversation queued execution path as the prompt it represents.
...
- Every new Slack conversation starts with an empty RocketCode session. Its initiating human, External MCP, tool-created, or scheduled prompt is its first local turn. Later turns replay exclusively that conversation's own persisted history.
