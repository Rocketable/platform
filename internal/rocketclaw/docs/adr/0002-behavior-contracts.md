...
- `ask_user_question` asks the authorized human in the originating Slack or Discord Text conversation using in-message native UI only: Slack Block Kit messages and Discord message components. It does not use modals. Choice answers use native buttons or selects. Custom/free-text answers consume the next authorized reply in the same connector conversation. Calls must provide at least one answer path: one or more native choice options, or `allow_custom` for a custom/free-text answer. Calls with no options and `allow_custom` false must fail without creating a pending question. Unauthorized users must not resolve pending questions.
