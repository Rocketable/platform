...
- Scheduled cron final verbatim output is submitted to the configured cron text delivery sink. With an enabled primary text connector, it starts a managed conversation through that connector: if `channel` is present, that value is the target; otherwise the target is the primary text connector's configured default Slack DM room or Discord guild channel. With no enabled primary text connector, the inert delivery sink accepts the output and the no-`channel` main-session internalization path still runs. `slack-channel` remains a backward-compatible alias. Replies and summaries for cron-created connector conversations follow existing connector gates.
