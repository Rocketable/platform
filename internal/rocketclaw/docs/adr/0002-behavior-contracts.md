...
- Text-visible RocketCode subagent progress diagnostics include a stable per-dispatch ordinal immediately after `subagent`, formatted as `(n/total)`, including `(1/1)` when a model response dispatches exactly one subagent task.
...
- `ask_user_question` asks the authorized human in the originating Slack or Discord Text conversation using in-message native UI only: Slack Block Kit messages and Discord message components. It does not use modals. Choice answers use native buttons or selects. Custom/free-text answers consume the next authorized reply in the same connector conversation. Unauthorized users must not resolve pending questions.
