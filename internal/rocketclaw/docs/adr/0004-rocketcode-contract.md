...
- RocketClaw treats OpenAI, OpenAI-compatible, and Anthropic as peer RocketCode provider families. Both persistent-bridge and raw-run construction paths must preserve the same product, operational, and internal RocketCode capabilities across provider families unless an ADR explicitly exempts a provider family from a specific capability.
- RocketClaw constructs provider clients according to the loaded agents' declared models and the runtime paths that are enabled. It passes the selected named OpenAI-compatible providers into RocketCode unchanged, passes an Anthropic client when any selected agent model requires Anthropic or Anthropic credentials are configured, and passes a first-party OpenAI client only when a selected RocketCode path requires first-party OpenAI. Provider client construction must not make first-party OpenAI a global prerequisite for Anthropic-only or OpenAI-compatible-only deployments.
- Model strings may use `openai/<model>`, `anthropic/<model>`, or `openai-compatible/<provider>/<model>` in agent frontmatter and runtime defaults. Unprefixed model strings and empty model reference parts are construction errors. Every loaded agent must declare non-empty `model` frontmatter; runtime/default models do not fill missing agent model declarations. RocketClaw startup validates every loaded agent's declared model against configured providers in both persistent-bridge and raw-run construction paths, including embedded, generated, configured-overlay, and workspace-overlay agents.
- ChatGPT OAuth applies only to first-party OpenAI provider requests. Anthropic provider requests use only the configured Anthropic API key and optional Anthropic base URL. OpenAI-compatible provider requests use only their selected top-level `openai_compatible` provider entry.
- Missing Anthropic credentials are an error only when a selected model requires Anthropic.
- Missing OpenAI-compatible provider configuration is an error only when a selected model requires that named compatible provider.
- Missing first-party OpenAI credentials are an error only when a selected resolved model or enabled path requires first-party OpenAI, such as an `openai/...` agent model, ChatGPT OAuth, or enabled OpenAI-backed audio.
- Hosted OpenAI tools, including hosted `websearch`, are not exposed to Anthropic or OpenAI-compatible model requests. Equivalent shared tool capabilities must still be provided through local tools or provider adapters when the capability is part of RocketCode's shared tool surface.
...
- Response checkpoint seeding through replay compaction is a shared RocketClaw/RocketCode provider-family capability. It supports first-party OpenAI Responses, OpenAI-compatible providers, and Anthropic providers with product, operational, and internal capability parity. The purpose of this step is to seed a new conversation from prior main-session context without making the human repeat context, regardless of which provider produced the original checkpoint. OpenAI-compatible `responses` mode uses that provider's Responses compaction endpoint. OpenAI-compatible `chat_completions` mode uses RocketCode client-side summary compaction with the same selected provider and model. Anthropic seeding uses Anthropic-compatible summarization and replay projection rather than OpenAI Responses compaction. Provider-specific mechanics must sit behind shared abstractions or equivalent construction contracts that force every configured provider family through the same seeding capability. Unsupported provider branches are allowed only when an ADR explicitly removes that provider from the capability and states the user-visible, operational, and internal consequences.
