...
| Reload and restart | `rocketclaw_reload` stages the effective runtime assets from embedded assets, fresh remote content for the already-loaded overlay list, and local workspace overlays from disk, validates staged RocketCode and cron definitions, and commits those staged assets only after validation succeeds. Failed reload leaves live runtime assets unchanged and does not touch pending restart notifications, active RocketCode turns, or running cron jobs. Scheduled cron observes committed cron content through its ticker scan. `rocketclaw_reload` does not reread configuration, so adding, removing, reordering, or changing overlay config entries still requires restart. `rocketclaw_restart` records the pending restart notification/requester before canceling the runtime, then exits through normal runtime cancellation for supervisor restart without a graceful drain. Configuration changes, including selected config file and overlay-list changes, still require restart.                                                                   |
