...
| Slack text binding | Slack uses Socket Mode, app mentions, Slack DMs, and Slack threads for managed and response-rooted conversations. For a thread shared through Slack, the sender's message is the request and the shared material is reference, not instructions. RocketClaw uses Slack's visible preview once. It reads the full thread only when Slack clearly identifies the shared thread and RocketClaw's Slack app confirms that the source channel is public. ADR 0002 defines the detailed limits and safety rules. Slack social-mode channel replies that ping other people, bots, broadcast targets, or user groups are suppressed unless the RocketClaw bot is also mentioned; Slack channel references do not trigger that suppression. Emoji-prefixed prompts can start managed agent threads, Slack goal triggers use the same connector-local emoji-prefix normalization as other emoji-prefixed starts before entering the shared goal parser, and allowed `rocketclaw_start_new_thread` tool calls create a new bot-authored top-level root message in the originating Slack channel or DM with no parent thread timestamp and manage a Slack thread rooted at that message. `:floppy_disk:` summarizes managed threads back to main. |
| External MCP       | `mcp_external` serves HTTP `/mcp` with exactly one tool, `session_prompt`, accepting input, optional external conversation ID, required agent for new conversations, optional Slack channel, optional metadata, and optional inbound attachments. New external MCP conversations use the requested loaded agent; existing external MCP conversations use the agent already persisted for that external conversation ID, ignoring and logging any mismatched requested agent. External MCP relays the prompt and any MCP-provided attachments into the primary text connector as one visible native text-and-files message, and returns the final answer, the agent used for the turn, and outbound attachments produced by the shared response attachment path. External MCP active-turn recovery is keyed by the private durable RocketClaw/RocketCode conversation ID, not only by the public external conversation ID. If the process restarts, the old blocking `session_prompt` HTTP/MCP request cannot be completed by recovery; later same-conversation follow-ups resolve to the same private conversation and queue behind the recovered turn. `rocketclaw_start_new_thread` is never exposed through External MCP or any other MCP-originated turn.                          |
| Cron               | `cron/*.md` files are loaded at startup, and scheduled cron uses one global ticker loop that scans effective `<runtime-dir>/cron/` definitions to observe committed scheduled cron definition changes at the best next scheduling or run-start opportunity without rereading runtime configuration or restarting. Scheduled cron execution state is persisted in `<runtime-dir>/state.sqlite3`, with per-file serialization and no missed-trigger backlog replay; one-off cron requests keep their current live-file loading and direct execution behavior. Scheduled and one-off cron prompts run through raw RocketCode runs. Every completed scheduled cron adds a silent run summary to `main`. The `rocketclaw_i_want_human_partner_to_see_this` argument is separate extra output for the human: an empty string sends no connector output, and a non-empty string opens or uses the managed cron output thread for that text. Cron output threads use the configured `channel` when present, otherwise the primary text connector's default Slack DM room when Slack is enabled. Extra human-visible cron output is never delivered as main-session verbatim output. Replies and summaries for cron-created connector conversations follow the connector's existing gates. Cron-originated turns, including raw, scheduled, one-off, and cron-created conversation continuations, never receive `rocketclaw_start_new_thread`. `*.example.md` files are ignored.                  |
