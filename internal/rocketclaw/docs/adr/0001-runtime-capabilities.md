...
| Cron               | `cron/*.md` files are loaded at startup. They can run scheduled or one-off prompts through raw RocketCode runs and can produce internal main-session notes or managed primary text connector conversation output. Scheduled cron visible output uses the configured `channel` when present, otherwise the primary text connector's default Slack DM room or Discord guild channel when a primary text connector is enabled; no-`channel` scheduled cron results also keep the existing main-session internalization path. Replies and summaries for cron-created connector conversations follow the connector's existing gates. Cron-originated turns, including raw, scheduled, one-off, and cron-created conversation continuations, never receive `rocketclaw_start_new_thread`. `*.example.md` files are ignored.                  |
