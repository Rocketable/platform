...
| Slack social mode  | Configured app mentions in allowed Slack channels can start channel-scoped conversations for configured agents. Social-mode channel authorization uses the configured channel's per-channel allowed users when non-empty, otherwise the top-level social-mode allowed users. Replies in social-mode-gated Slack channel threads are normally consumed, but Slack-resolved direct user or bot mentions, broadcast targets, and user groups are skipped unless the RocketClaw bot is also mentioned. Slack channel references do not trigger this skip. Unconfigured channels are ignored.                                                                                           |
