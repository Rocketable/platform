...
| Restart            | `rocketclaw_restart` schedules a graceful restart, stops intake, drains queues and active bridges, records pending restart notifications, and exits for supervisor restart. Configuration changes, including selected config file and overlay-list changes, still require restart.                                                                   |
