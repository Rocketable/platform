...
| Slack text binding | Slack uses Socket Mode, app mentions, and Slack threads for managed conversations. Every new Slack conversation starts with an empty RocketCode session; its initiating human, External MCP, tool-created, or scheduled prompt is its first local turn, and later turns replay only that conversation's own history. For a thread shared explicitly through Slack, the sender's message is the request and the shared material is current-turn reference. RocketClaw uses Slack's visible preview once and reads the full shared thread only when Slack clearly identifies it and the Slack app confirms that the source channel is public. ADR 0002 defines the detailed limits and safety rules. Channel replies that ping other people, bots, broadcast targets, or user groups are suppressed unless the RocketClaw bot is also mentioned; Slack channel references do not trigger that suppression. Allowed `rocketclaw_start_new_thread` tool calls create a fresh bot-authored top-level root in the originating configured channel. |
| External MCP       | `mcp_external` serves HTTP `/mcp` with exactly one tool, `session_prompt`, accepting input, optional external conversation ID, required agent for new conversations, required configured Slack channel on every turn, optional metadata, and optional inbound attachments. The first turn creates one fresh Slack thread and binds the external conversation ID to that thread and its persistent conversation history. Later turns for the same external conversation ID must supply the same configured Slack channel, relay into the same thread, and queue in the same conversation alongside authorized Slack replies; omitted or changed channels are rejected. Separate external conversation IDs remain isolated even when they use the same Slack channel. New conversations use the requested loaded agent; existing conversations use their persisted agent, ignoring and logging any mismatched requested agent. External MCP returns the final answer, the agent used for the turn, and outbound attachments produced by the shared response attachment path. Active-turn recovery is keyed by the durable bound conversation ID. If the process restarts, the old blocking `session_prompt` request cannot be completed by recovery; later same-conversation follow-ups queue behind the recovered turn. `rocketclaw_start_new_thread` is never exposed through an MCP-originated turn. |
