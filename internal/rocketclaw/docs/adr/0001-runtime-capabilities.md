...
| Slack social mode  | Configured app mentions in allowed Slack channels can start channel-scoped conversations for configured agents. Social-mode channel authorization uses the configured channel's per-channel allowed users. Replies in social-mode-gated Slack channel threads are normally consumed, but Slack-resolved direct user or bot mentions, broadcast targets, and user groups are skipped unless the RocketClaw bot is also mentioned. Slack channel references do not trigger this skip. Unconfigured channels are ignored.                                                                                           |
...
| Slack goal loops   | Slack DM `🔁`/`🏁` prompts, Slack social-mode `@BotName 🔁`/`@BotName 🏁` mentions, and authorized `🔁`/`🏁` messages inside existing managed Slack threads can start persisted conversation-local goal loops with optional `maxTurns:` budgets and optional `checkScript:` completion gates, as specified by ADR 0007.                    |
| Discord text       | Discord text is a product-peer primary text connector to Slack DM. It connects configured Discord text channels and DMs, accepts configured-human messages, routes the main conversation, publishes visible thinking/progress and final responses, can host managed guild-thread conversations, and mirrors Slack social-mode channel-agent mentions and per-channel authorization where Discord channels can express them. Discord text remains mutually exclusive with Slack DM as the primary text connector. |
| Discord goal loops | Discord text `🔁`/`🏁` prompts, social-mode bot mentions, DMs, and authorized messages inside existing managed Discord conversations can start persisted conversation-local goal loops with Slack-peer objective, `maxTurns:`, `checkScript:`, visible-progress, interruption, restart, and terminal-status behavior adapted to Discord delivery surfaces.                    |
...
| Attachments        | Supported image attachments can be passed into RocketCode. Text attachments from Slack and external MCP are converted into prompt text within size limits. External MCP accepts base64 attachments that flow through the same inbound attachment handling as connector-provided attachments. RocketCode response attachments flow through the shared outbound attachment carrier used by Slack delivery and external MCP result rendering. |
...
- 2026-06-14: Updated Slack goal-loop capability for conversation-local goals, existing-thread starts, and check scripts; added Discord Text parity direction for social mode, DMs, visible progress, and goal loops.
