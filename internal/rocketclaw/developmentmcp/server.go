...
func Start(ctx context.Context, logger *slog.Logger, listenAddr string, users map[string]string, overlaySpecs []string, readOverlayContext func(spec string) (skel.OverlayContext, error), lint func(baseOverlay string, files []skel.OverlayFile) (agentlint.Result, error), runTurn func(ctx context.Context, baseOverlay string, files []skel.OverlayFile, agent, prompt, conversationID string) (thinking, answer string, err error), reload, restart func(reason string) (string, error)) (*Server, error) {
	if len(users) == 0 {
	...
	mcp.AddTool(mcpServer, &mcp.Tool{Name: lintToolName, Description: "Lint the overlay represented by the given context."}, func(_ context.Context, _ *mcp.CallToolRequest, input lintInput) (*mcp.CallToolResult, []lintFinding, error) {
		files := make([]skel.OverlayFile, len(input.Context.Files))
		for i, file := range input.Context.Files {
			files[i] = skel.OverlayFile{Path: file.Path, Content: file.Content}
		}
