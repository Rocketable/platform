...
- Before running tests, inspect the actual touched diff for every Go standard violation: error variable names (`errCombined`, `errRead`, `errClose`, not `combinedErr`, `readErr`, `closeErr`), error type names ending in `Error`, single-use helpers to inline, all single-use single-line functions and one-line delegating wrappers to delete by inlining, defensive guards to delete, accidental abstraction growth, mutex-hat placement, context misuse, exported names, changed-line necessity, queue/order semantics for message flow, and source CLOC impact. Fix the diff first; do not use tests as a substitute for this review.
