...
func TestSyncResetsTargetPreservingState(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(root, "state.sqlite3.lock"), []byte("lock"), 0o600))
	...
	data, err = os.ReadFile(filepath.Join(root, "state.sqlite3-shm"))
	require.NoError(t, err)
	assert.Equal(t, "shm", string(data))
	data, err = os.ReadFile(filepath.Join(root, "state.sqlite3-wal"))
	require.NoError(t, err)
	assert.Equal(t, "wal", string(data))
	data, err = os.ReadFile(filepath.Join(root, "state.sqlite3.lock"))
	require.NoError(t, err)
